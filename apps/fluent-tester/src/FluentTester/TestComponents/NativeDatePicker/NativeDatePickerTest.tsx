...
const nativeDatePicker: React.FunctionComponent<{}> = () => {
  ...
  const titles = {
    ...
    <Stack style={stackStyle}>
      <Button content="Date picker" onClick={() => NativeDatePicker.presentWithParameterObject({})}/>
      <Button content="Date time picker" onClick={() => NativeDatePicker.presentWithParameterObject({mode: "dateTime"})}/>
      <Button content="Preselected date" onClick={() => NativeDatePicker.presentWithParameterObject({startDate: "2020-02-29T12:00:00.000Z"})}/>
      <Button content="Date range (tabbed)" onClick={() => NativeDatePicker.presentWithParameterObject({mode: "dateRange"})}/>

      {/* Shows "Date Title" and "Date Subtitle" only.  Other title args are ignored. */}
      <Button content="Date with custom titles" onClick={() => NativeDatePicker.presentWithParameterObject({...dates, ...titles})}/>

      {/* Shows "Date Title" and "Date Subtitle" on the first page;
          shows "Time Title" and "Time Subtitle" on the second page. */}
      <Button content="Date time with custom titles" onClick={() => NativeDatePicker.presentWithParameterObject({mode: "dateTime", ...dates, ...titles})}/>

      {/* Shows "Start Title" and "Start Subtitle" on the first page;
          shows "End Title" and "End Subtitle" on the second page. */}
      <Button content="Date range with custom titles (paged)" onClick={() => NativeDatePicker.presentWithParameterObject({mode: "dateRange", dateRangePresentation: "paged", ...dates, ...titles})}/>

      {/* Shows "Date Title", "Date Subtitle", "Start Tab" and "End Tab". */}
      <Button content="Date range with custom titles (tabbed)" onClick={() => NativeDatePicker.presentWithParameterObject({mode: "dateRange", ...dates, ...titles})}/>

      {/* NOTE: The paged presenetation mode of the date time range picker is bugged.  It shows a time picker with start and end tabs. */}
      {/* <Button content="Date time range with custom titles (paged)" onClick={() => NativeDatePicker.presentWithParameterObject({mode: "dateTimeRange", dateRangePresentation: "paged", ...dates, ...titles})}/> */}
      ...
      <Button content="Date time range with custom titles (tabbed)" onClick={() => NativeDatePicker.presentWithParameterObject({mode: "dateTimeRange", ...dates, ...titles})}/>
    </Stack>
  );
};
