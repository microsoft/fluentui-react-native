...
`*.stories.(ts|tsx)` file from the agentic library source (`packages/agentic-components/src`) plus
the standalone native packages so their stories run in the Fabric host.
