...
export const useInput = (props: InputProps): InputInfo => {
  const { label, secondaryText, assistiveText, onBlur, onFocus, onChange, onPress, value, defaultValue, ...rest } = props;
  const pressable = usePressableState({ onBlur, onFocus });
  const [text, setText] = React.useState<string>(defaultValue ? defaultValue : '');
  return {
    props: { ...pressable.props, label, secondaryText, assistiveText, setText, onChange, onPress, value, defaultValue, ...rest },
