 ...
 *     function foo<T>(x: T, y: GetTypedMemoValue<T>)
 ...
 */
export type GetMemoValue = <T>(factory: FactoryType<T>, keys: unknown[]) => MemoResult<T>;
export type GetTypedMemoValue<T> = (factory: FactoryType<T>, keys: unknown[]) => TypedMemoResult<T>;

/** type helper for parameters that accept typed or untyped memo values */
export type AnyGetMemoValue<T> = GetTypedMemoValue<T> | GetMemoValue;
 ...
 */
export type MemoResult<T> = [T, GetMemoValue];
export type TypedMemoResult<T> = [T, GetTypedMemoValue<T>];
 ...
 */
function getMemoValueWorker<T>(entry: CacheEntry, factory: T | ValueFactory<T>, keys: unknown[]): [T, GetTypedMemoValue<T>] {
 ...
 */
export function getMemoCache(globalKey?: MemoObjectKey): GetMemoValue {
 ...
 * to retrieve the same cache from the global call.  If not specified the returned cache will be completely isolated.
 */
export function getTypedMemoCache<T>(globalKey?: MemoObjectKey): GetTypedMemoValue<T> {
  return getMemoCache(globalKey) as GetTypedMemoValue<T>;
}

/** type coercion helper */
export function asTypedMemoCache<T>(cache: AnyGetMemoValue<T>): GetTypedMemoValue<T> {
  return cache as GetTypedMemoValue<T>;
}

/** type coercion helper */
export function asGenericMemoCache<T>(cache: AnyGetMemoValue<T>): GetMemoValue {
  return cache as GetMemoValue;
}
