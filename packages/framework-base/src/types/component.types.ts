...
import type { IsSingleton } from './utility.types';
...
/**
 * Declaration helper, avoids union types and adds null as a sentinel for optionality.
 */
type DeclareSlot<Type extends React.ComponentType> =
  IsSingleton<Extract<Type, string>> extends true
    ? Type | null
    : 'Error: Slot type must not be a union of types. See documentation of Slot type.';

/**
 * Declare a required slot for a component, removing the null sentinal type
 */
export type Slot<Type extends React.ComponentType> = NonNullable<DeclareSlot<Type>>;

/**
 * Declare an optional slot for a component
 */
export type OptionalSlot<Type extends React.ComponentType> = DeclareSlot<Type>;
...
type PropsOfSlot<T extends React.ComponentType | null> = PropsOf<NonNullable<T>>;
 ...
 */
export type SlotProp<Type> = WithSlotShorthandValue<PropsOf<Type>> | ({ as?: Type } & PropsOf<Type>);

/**
 * Maps the slot props type, which is a record of component types, to a record of the props types for each slot.
 */
type SlotPropsToComponentProps<Slots extends SlotPropsRecord> = {
  [K in keyof Slots]: PropsOfSlot<Slots[K]>;
};
 ...
 */
type NonRootSlotPropsToComponentProps<Slots extends SlotPropsRecord> = SlotPropsToComponentProps<Omit<Slots, 'root'>>;
 ...
 * @internal
 */
export type SlotPropsRecord = Record<string, React.ComponentType | null>;
...
export type OptionalSlotProp<Type> = SlotProp<Type> | null;
 ...
 * @internal
 */
type RequiredOrOptionalSlot<T extends React.ComponentType | null> = T extends null
