...
import type { IsSingleton, ReplaceNullWithUndefined } from './helpers';

// temporary for testing purposes
export const SLOT_CLASS_NAME_PROP_SYMBOL = Symbol('SLOT_CLASS_NAME_PROP');
export const SLOT_ELEMENT_TYPE_SYMBOL = Symbol('SLOT_ELEMENT_TYPE');
export const SLOT_RENDER_FUNCTION_SYMBOL = Symbol('SLOT_RENDER_FUNCTION');
  ...
  IsSingleton<Extract<Type, string>> extends true
    ?
        | WithSlotShorthandValue<
          ...
          >
        | (AlternateAs extends unknown ? { as: AlternateAs } & WithSlotRenderFunction<IntrinsicElementProps<AlternateAs>> : never)
        | null
 ...
 */
export type EventHandler<TData extends EventData<string, unknown>> = (ev: React.SyntheticEvent | Event, data: TData) => void;
