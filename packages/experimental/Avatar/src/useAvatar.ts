...
export const useAvatar = (props: JSAvatarProps): AvatarInfo => {
  const { active, accessibilityLabel, activeAppearance, badge, name, src, ring, shape, ...rest } = props;
...
export const getInitials = (name: string): string => {
  ...
  const nonWordRegExp = /\W+(?<!\S-)/g;
  const wordsInBracesRegExp = /(\(|\[|\{)\w+(\)|\]|\})/g;
...
export const validateAlphabeticalCharacters = (name: string): boolean => {
  const alphabeticalRegExp = /[a-zA-Z]/g;
