...
export function shallowCompare<A extends object, B extends object>(a: A, b: B, paths: string[] = []): void {
  if (Object.keys(a).length !== Object.keys(b).length) {
    throw new Error(`Shallow compare found two nodes at ${paths.join(': ')} with different property counts`);
  }
  Object.keys(a).forEach((key) => {
    if (key !== 'children' && a[key] !== b[key]) {
...
export function renderTwiceAndCompare(render: JSXProducer, depth: number = 1) {
