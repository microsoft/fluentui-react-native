...
class AppleThemeModule: NSObject {
	...
	func colorWithEffect(color: NSColor?,
		...
		if let color = color {
			let systemEffect = stateToSystemEffectDictionary[effect] ?? .none;
			let colorWithAppliedEffect = color.withSystemEffect(systemEffect)
			let hexString = RCTColorToHexString(colorWithAppliedEffect.cgColor);
...
//		let neutralBackground3 = NSColor(named: "ButtonColors/neutralBackground3", bundle: FluentUIResources.resourceBundle)!
		...
		return [
			...
			"brandDisabledBackground" : RCTColorToHexString(brandBackgroundDisabled.withSystemEffect(.disabled).cgColor),
