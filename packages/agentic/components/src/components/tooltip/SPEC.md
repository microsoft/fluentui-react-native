...
`root` is a passive inline wrapper. `trigger` is a public `Pressable` slot narrowed to presentation: children, style, test identifier, accessible name, ARIA-aligned `role`, a `ref`, and the pointer, focus, and press handlers, each of which runs after Tooltip's own handling. Tooltip owns the trigger's description, accessibility state, accessible flag, disabled state, focusability, and native focus ring, and composes the consumer `ref` with the internal anchor ref. `content` is a public `Text` slot; its user style is applied after the tokenized label style.
...
- **TIP-005:** Draw the label surface — fill, corner radius, clipping, asymmetric padding, and the native width guard — with no stroke on the React Native content host that both platforms render, apply the Flex label typography and foreground, and render persistent trigger focus feedback through `FocusVisual`.
...
That content host stays between 160 and 240 points wide. The minimum prevents the native callout from measuring unconstrained label text as a tall one-character column, while the Fluent v9 maximum keeps longer descriptions compact.
