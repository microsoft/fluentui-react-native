...
Hover and press states change the token-derived colors. The root follows the
[shared focus visual policy](../../AGENTS.md#focus-visual-policy): request native
feedback on every platform with the persistent FocusVisual hidden, or use the existing custom
visibility calculation on the retained path. There is no component-owned timed
animation.
