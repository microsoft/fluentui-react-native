...
- **WCAG:**
  ...
  - **2.4.7 — Focus Visible:** The universal persistent dual-ring focus visual (see `flex-system:styling`) must render on the row whose interactive element matches `:focus-visible`. In composite mode, inner focusables (Action items) render their own focus rings independently.
