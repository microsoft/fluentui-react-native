...
The overlay's focus feedback follows the [shared focus visual policy](../../AGENTS.md#focus-visual-policy); the persistent `FocusVisual` remains decorative and is hidden on the system path on every platform. Native rendering depends on platform support. On Windows, the interactive overlay maps to a UI Automation button; on macOS, it maps to an AX button. Nested controls supplied through slots are separate controls and must have their own names and semantics.
