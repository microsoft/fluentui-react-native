...
describe('Input', () => {
  ...
  it.each([
    ...
    const requestFocus = jest.spyOn(observed.state!.focusTarget, 'requestFocus');
    await fireEvent.press(getContents(component));
    if (props.disabled) {
      expect(requestFocus).not.toHaveBeenCalled();
    } else {
      expect(requestFocus).toHaveBeenCalledWith('pointer');
      expect(requestFocus.mock.results[0].value.status).toBe(expected ? 'requested' : 'not-focusable');
    }
    await fireEvent(getTextbox(component), 'focus', {});
    expect(observed.state?.focused).toBe(expected);
    if (expected) {
      expect(observed.state?.focusTarget.getSnapshot().intent).toBe('pointer');
    }
  });
