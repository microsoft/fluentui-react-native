...
| `input-underline-double-stroke` | Deferred    | For the underline variant the contents row carries a thin bottom border and a separate absolutely positioned underline view carries another one, so the bottom edge is drawn twice. Flex describes a single stroke whose surface area is the only difference between variants. | Pick one owner for the bottom edge and remove the redundant stroke.                                                                               |
