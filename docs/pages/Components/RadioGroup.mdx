...
## Purpose: The goal of this RadioGroup component is to let users select one option
            from two or more choices. Each option is represented by one RadioButton
            component, and the group of RadioButton’s is represented by a RadioGroup
            component. A user can only select one RadioButton in a RadioGroup.
...
## Tokens:
  ...
  RadioGroup supports the following tokens:
    ...
    2.  color - This changes the text color of the label associated with the RadioGroup.
